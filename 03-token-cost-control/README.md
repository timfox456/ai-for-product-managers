...
**Duration:** 30 Minutes
**Tools:** ChatGPT or Microsoft Copilot, Calculator (or spreadsheet), any text editor with word count
...
- 100 tokens ≈ 77 words
- 1,000 tokens ≈ 770 words (about 1.5 pages)
...
Total: ~660 tokens for one interaction
```
...
*Note: Dollar amounts above are illustrative. Actual costs depend on which model you use and current pricing — see the reference table at the bottom of this lab and always check the provider's current pricing page.*
...
- Use the most capable model (e.g., GPT-4 class) for complex reasoning
- Use a cheaper model (e.g., GPT-3.5 class) for simple tasks — often 10-20x cheaper
- Use the least capable model that can do the job acceptably
...
- Use the `max_tokens` parameter to cap responses

**Strategy 4: Batch Operations**
- Combine multiple user requests into a single AI call
...
**Strategy 6: Cache Repeated Context**
- If your system prompt or a reference document is sent with every request, both OpenAI and Anthropic offer **prompt caching** that reduces costs on repeated prefixes by 50–90%
- This is a significant optimization for any product with a large, stable system prompt — worth flagging to your engineering team early

---

## Step 1: Estimate Your Token Budget (10 Mins)
...
**In ChatGPT or Copilot, paste this prompt:**
...
Using the conversion rate of 1 word ≈ 1.3 tokens, please calculate:
1. Estimated input tokens (all context sent to AI)
2. Estimated output tokens (AI response)
3. Total tokens per interaction
4. Cost per interaction at $0.03/1K input tokens, $0.06/1K output tokens
5. Monthly cost if we have 5,000 support agents, each using this 10x per day
6. Monthly cost if we switch to a model at $0.001/1K input, $0.002/1K output

Show your math clearly for each step.
```

> **Important:** After the AI gives you numbers, verify the final calculations yourself with a calculator. LLMs can make arithmetic errors — and catching one here is itself a valuable PM habit. The token-to-word conversion and the multiplications are simple enough to spot-check in under a minute.

### The Deliverable

**Copy the AI's calculation, verify the math, then add this analysis:**
...
Calculated Cost Per Interaction (higher-cost model): $[amount]
Monthly Cost (5K agents, 10x/day, higher-cost model): $[amount]
Monthly Cost (same usage, lower-cost model): $[amount]
...
- Higher-cost model gives [quality level] but costs [cost]
- Lower-cost model gives [quality level] but costs [cost]
...
**Goal:** Learn to identify and eliminate token inefficiencies in prompts — without sacrificing output quality.
...
→ 72 words ≈ 94 tokens (waste: ~40 tokens of filler)
```
...
Paste the bloated prompt into any word processor or character counter:
- Word count: ______
- Estimated token count (multiply by 1.3): ______

**Step 2: Ask AI to optimize it**

In ChatGPT or Copilot, paste:
...
**Step 3: Test whether quality is preserved**

Open a separate chat. Run both the original prompt and the optimized prompt with this simulated ticket:

> *"I was charged twice for my subscription last month — $49.99 appeared twice on my credit card statement on March 3rd. I emailed support two weeks ago and never heard back."*

Compare the two responses. Does the optimized prompt produce a response of similar quality? Note any differences in tone, completeness, or customer impact.
...
- Cost per call (higher-cost model @ $0.03/1K): $[amount]
...
QUALITY COMPARISON:
- Original output quality: [Rate 1-10]
- Optimized output quality: [Rate 1-10]
- Key differences noticed: [any gaps or losses?]
- Would you deploy the optimized version? [Yes / No / With modifications]

WHAT I LEARNED:
[Write 2-3 sentences about what made the original bloated and your
takeaways for writing token-efficient prompts going forward]
```
...
*(Hint: Teams often write prompts like emails — polite, contextual, explanatory. But AI doesn't need pleasantries. A review step specifically for "token efficiency" before prompts go to production — similar to a code review — could catch this. Who on your team should own that review?)*

---

## Step 3: Design a Usage Limiter (10 Mins)

**Goal:** Design product mechanisms to control costs while maintaining user experience.

### The Concept: The "Guardrails" Approach

You can't let users consume unlimited AI tokens — costs will be unmanageable. But you also can't be arbitrarily restrictive — users will churn.
...
A technical restriction (hard cutoff at X tokens) feels arbitrary and generates support tickets. A productized limit with clear tiers and benefits feels like choice and control. Same backend implementation, radically different user experience.
...
- Heavy users (10%) should have a clear upgrade path
- Must be transparent and fair
- Can't compromise core product value for free-tier users (avoid a
  "crippled free tier" that feels worthless — the goal is a useful
  free tier that heavy users genuinely want to upgrade from)
...
Output: Specific recommendations for each section. Include example
...
**Discussion:** Why is it better to design token limits as a **product feature** rather than just a **technical restriction**?

*(Hint: Users accept constraints when they understand them and see value. "You've used 80% of your monthly analysis budget — upgrade to Pro for unlimited" is a product experience. "Error: quota exceeded" is a technical error. Same backend implementation, radically different user reaction.)*
...
- **Token Math** — How to estimate costs before building features, and how to verify them
- **Prompt Efficiency** — How to audit and optimize token waste without sacrificing quality
- **Limit Design** — How to build guardrails that users accept and that support your business model

### Token Checklist for Every AI Feature:

**Before Building:**
- [ ] Estimate tokens per user interaction
- [ ] Calculate cost at projected scale (and verify the math manually)
- [ ] Model unit economics (AI cost vs. revenue per user)
- [ ] Decide on model tier — use the cheapest model that meets quality bar
- [ ] Check if prompt caching applies (repeated system prompts or reference docs)
...
**The Challenge:** Take a real AI-powered feature and put it on a "token diet" — then test whether the diet version actually holds up.
...
3. **Optimize aggressively** — Rewrite to achieve the same results with 50% fewer tokens

4. **Test both versions** — Run both prompts with the same (brief) interview excerpt, compare output quality. Does the diet version miss anything?
...
- Key strengths of output: [what it did well]
...
[Did the token diet hurt output quality? By how much? Is it worth it?]
...
- **Verify the Math** — AI-generated calculations can contain errors; spot-check them
- **Prompt Efficiency** — Bloated prompts are silent cost killers; audit before production
...
After: "Let's add AI to this feature. It'll use ~2K tokens per call, cost roughly $0.06 per user action, so at 10K daily active users we're looking at significant monthly spend. We can afford that if we charge $29/month — let me verify that math and put it in the PRD."
...
### Simple Formula:
```
Monthly Cost = Users × Actions/User/Day × Days × Tokens/Action × Price/Token
```
...
Price per 1K tokens: $0.03 (illustrative)

Calculation:
5,000 × 10 × 30 × 1,000 = 1.5 billion tokens/month
1.5B ÷ 1,000 = 1,500,000 (thousands of tokens)
1,500,000 × $0.03 = $45,000/month
```

### Model Pricing Reference:

> **Important:** AI pricing changes frequently. The figures below are illustrative order-of-magnitude estimates. Always check the provider's current pricing page before doing real cost modeling.
>
> - OpenAI pricing: platform.openai.com/docs/pricing
> - Anthropic pricing: anthropic.com/pricing

| Model Class | Input (per 1K tokens) | Output (per 1K tokens) | Use Case |
|-------------|----------------------|------------------------|----------|
| GPT-4 class (complex) | ~$0.01–0.03 | ~$0.03–0.06 | Complex reasoning |
| GPT-3.5 / GPT-4o-mini class | ~$0.001–0.002 | ~$0.001–0.002 | Simple tasks |
| Claude Opus class | ~$0.015 | ~$0.075 | Long context, complex |
| Claude Sonnet class | ~$0.003 | ~$0.015 | Balanced quality/cost |
